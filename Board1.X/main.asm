;============================================================
; main.asm
; Target  : PIC16F877A  (HS OSC varsayimi)
; Fosc    : 4MHz kabul edilerek Timer1 preload hesaplari yapildi
;
; I/O Haritasi:
;  RA0/AN0  : LM35 analog giris
;  RA4/T0CKI: Fan tach/pulse girisi (TMR0 external counter)
;  RB0..RB3 : Keypad kolon (output)
;  RB4..RB7 : Keypad satir (input + pullup)
;  RC0..RC3 : 7-seg digit enable (multiplex)
;  RD0..RD7 : 7-seg segment hatlari (a..g,dp)
;  RC6/TX   : UART TX
;  RC7/RX   : UART RX
;  RE0      : Heater output
;  RE1      : Cooler output
;
; Yazilim Mimarisi:
;  - ISR sadece Timer1 (100ms) ve opsiyonel RBIF servisler
;  - MainLoop: Display_Service + UART_Service + Keypad_Poll
;              + flag'lere gore ADC okuma / display rebuild
;============================================================
...
;------------------------------------------------------------
; ISR (0x0004)
; - W/STATUS/PCLATH context save/restore yapilir
; - TMR1IF: Temp_TMR1_ISR (100ms tick -> 1s olaylari)
; - RBIF : Keypad_RB_ISR (RB port mismatch temizleme)
; Not: ISR icinde uzun is yapmiyoruz; ana is main loop'a flag ile aktarilir.
;------------------------------------------------------------
...
MainLoop:
; Display_Service:
;  - Her loop'ta 1 digit yenilenir (multiplex)
;  - Gecikmesiz cagirilmasi titreme/kararma riskini azaltir

        CALL    Display_Service

; UART_Service:
;  - Blocking yapmadan RX buffer kontrol eder
;  - OERR/FERR durumlarini toparlar
;  - Projede UART ile hedef sicaklik guncelleme varsa burada flag setlenir

        CALL    UART_Service

; Keypad_Poll:
;  - Scan + debounce (key_lock) yapar
;  - key_ready=1 olursa Keypad_Process calisir
