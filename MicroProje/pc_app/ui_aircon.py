    ...
    while True:
        ...
        if conn.connected:
            try:
                ...
                print(f"Desired Temp : {desired:.1f} C" if desired is not None else "Desired Temp : -")
                print(f"Ambient Temp : {ambient:.1f} C" if ambient is not None else "Ambient Temp : -")
                print(f"Fan Speed    : {fan:.1f} rps" if fan is not None else "Fan Speed    : -")
